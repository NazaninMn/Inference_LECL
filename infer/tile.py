...
def _post_process_patches(
    ...
    probability_path = '../data/Inference/output_infer/probability_check/'
    if not os.path.exists(probability_path):
        os.makedirs(probability_path)
    ...
    ####Nazanin probability check
    # np.save(probability_path +str(image_info["name"])+'.npy',pred_map)
    # np.save(probability_path +str(image_info["name"])+'_ins.npy',pred_inst)
    ####Nazanin probability check
    overlaid_img = visualize_instances_dict(
        src_image.copy(), inst_info_dict, **overlay_kwargs
    )
#     np.save('/scratch/nm4wu/AHA/hover_net/dataset/probability_check/'+str(image_info["name"])+'_ins.npy',pred_map)
#     overlaid_img = visualize_instances_dict(
#         src_image.copy(), inst_info_dict, **overlay_kwargs
#     )
...
class InferManager(base.InferManager):
    ...
    def process_file_list(self, run_args):
        ...
        def proc_callback(results):
            ...
            print(pred_inst[pred_inst>0.1])
#             json.dump( mat_dict, open( "/scratch/nm4wu/AHA/data/Inference/output_infer/dict"+img_name+".json", 'w' ) )
...
#                 print('imgfilepath',type(img))
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) 
    
    
#                 print('rgb',img)
#                 mean_std=np.load('/scratch/nm4wu/AHA/WeaklySegPartialPoints/data_for_train/MO/mean_std.npy')*255   #TODO:Nazanin
#                 sub = np.subtract(img, mean_std[0])#TODO:Nazanin
#                 normal=np.divide(sub, mean_std[1])#TODO:Nazanin
#                 zeroone = (normal - normal.min())*255 / (normal.max() - normal.min())#TODO:Nazanin
#                 zeroone = zeroone.astype(np.uint8) #TODO:Nazanin
#                 img = np.clip(zeroone, 0, 255)#TODO:Nazanin
