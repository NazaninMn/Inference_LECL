...
def __proc_np_hv(pred):
    ...
    # processing
    blb = np.array(blb_raw >= 0.5, dtype=np.int32)  #TODO: Nazanin0.5 to 0.1
    
    blb = measurements.label(blb)[0]
    blb = remove_small_objects(blb, min_size=10) #remove20
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)) # creates kernel => conver 5 to 3
    marker = cv2.morphologyEx(marker, cv2.MORPH_OPEN, kernel) #removes noise (erosion followed by dilation that removes noise)
    marker = measurements.label(marker)[0]
    marker = remove_small_objects(marker, min_size=10) #remove20

    proced_pred = watershed(dist, markers=marker, mask=blb)
#     print('blb',proced_pred[proced_pred>0])
    return proced_pred, blb_raw
...
def process(pred_map, nr_types=None, return_centroids=False):
    ...
    if nr_types is not None:
        pred_type = pred_map[..., 3]   #TODO: Nazanin, change for contrastive learning
        pred_inst = pred_map[..., :3]   #TODO: Nazanin, change for contrastive learning
        pred_type = pred_type.astype(np.int32)
    else:
        pred_inst = pred_map

    pred_inst = np.squeeze(pred_inst)
    pred_inst, blb_raw = __proc_np_hv(pred_inst)
