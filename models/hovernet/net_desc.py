...
class HoVerNet(Net):
    ...
    def __init__(self, buffer_size, feat_dim, input_ch=3, nr_types=None, freeze=False, mode='original', proj='convmlp'):
        super().__init__()
        self.mode = mode
        self.freeze = freeze
        ...
        self.register_buffer("segment_queue", torch.randn(self.nr_types, self.r, self.feat_dim))
        self.segment_queue = nn.functional.normalize(self.segment_queue, p=2, dim=2)
        self.register_buffer("segment_queue_ptr", torch.zeros(self.nr_types, dtype=torch.long))

        self.register_buffer("pixel_queue", torch.randn(self.nr_types, self.r, self.feat_dim))
        self.pixel_queue = nn.functional.normalize(self.pixel_queue, p=2, dim=2)
        self.register_buffer("pixel_queue_ptr", torch.zeros(self.nr_types, dtype=torch.long))

        if proj == 'linear':
            self.proj = nn.Conv2d(64, self.feat_dim, kernel_size=1)
        elif proj == 'convmlp':
            self.proj = nn.Sequential(
                nn.Conv2d(64, self.feat_dim, kernel_size=1),
    ...
    def forward(self, imgs):
        ...
        for branch_name, branch_desc in self.decoder.items():
            ...
            if branch_name == 'tp':
                projection = F.normalize(self.proj(u1), p=2, dim=1)
                # {'seg': u0, 'embed': projection}
                out_dict[branch_name] = [u0, projection]
            else:
...
####
def create_model(buffer_size, feat_dim, mode=None, **kwargs):
    if mode not in ['original', 'fast']:
        assert "Unknown Model Mode %s" % mode
    return HoVerNet(buffer_size, feat_dim, mode=mode, **kwargs)
